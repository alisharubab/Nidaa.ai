...
| DATA-03 | Human baseline stopwatch run recorded | [x] | Ali, 2026-08-30 — real timed run completed: median 29940ms / p95 33753ms / n=6, live in `data/human_baseline.json` and already flowing through `/api/metrics` (`human_baseline_ms`), confirmed via curl, no restart needed. **Caveat, not a blocker:** this number is well under PRD §2.1's expected 90-300s range — likely because the dispatcher (Ali) already knew these 6 messages' content going in, answers were quick/rough rather than validated, and there was no map cross-reference or spreadsheet entry (the real friction PRD §1.2 describes). Treat as a placeholder; a colder, more representative run (ideally alongside `DATA-07`'s 25-message gold set) should replace it before this number goes in the pitch deck |
...
| CORE-25 | `POST /api/simulate` behind `DEMO_MODE` | [~] | Alisha — tested, `DEMO_MODE` gate works (403 when off). Currently inserts a canned ticket rather than replaying through the real pipeline, since the pipeline isn't wired yet — revisit once CORE-06..09 land |

...
| ID | Task | Done | Owner / Notes |
| :-- | :-- | :--: | :-- |
| CORE-01 | FastAPI app scaffold + health check | [ ] | |
| CORE-02 | `db.py`: tables from TRD §2 DDL, WAL pragmas, migration runner | [ ] | |
| CORE-03 | `events.py`: append-event helper, same-transaction writes | [ ] | |
| CORE-04 | Public routes stubbed with fixture JSON matching TRD §3.3 | [ ] | |
| CORE-05 | Groq smoke test (STT + LLM, hardcoded input) | [ ] | |
| ING-01 | Baileys socket bootstrap, pairing-code login | [ ] | |
| ING-02 | QR/pairing confirmed, inbound events logged | [ ] | |
| ING-03 | `POST /internal/ingest` stub calls against CORE-04 | [ ] | |
| FE-01 | Static dashboard shell (header/rails/map container) | [ ] | |
| FE-02 | Mock ticket list from static JSON fixture | [ ] | |
| DATA-01 | HDX COD-AB Pakistan gazetteer CSV built | [ ] | |
| DATA-02 | `aliases.json` seeded for top 40 districts | [ ] | |
| DATA-03 | Human baseline stopwatch run recorded | [ ] | |
| — | **Sync point:** schema (TRD §2) + API contract (TRD §3) frozen | [ ] | |
...
| CORE-10 | Real `GET /api/stream` SSE (replaces stub) | [ ] | |
| ING-04 | `media.js`: download + ffmpeg convert to 16kHz mono wav | [ ] | |
| ING-05 | Real `POST /internal/ingest` payload wired to real core routes | [ ] | |
| FE-03 | Real SSE client rendering live `ticket.created` events | [ ] | |
| FE-04 | Basic Leaflet map, one pin per ticket | [ ] | |
...
| ING-06 | `consent.js`: consent ledger state machine, BAND/STOP | [ ] | |
...
| CORE-21 | Real `GET /api/export/hxl.csv` | [ ] | |
...
| CORE-25 | `POST /api/simulate` behind `DEMO_MODE` | [ ] | |

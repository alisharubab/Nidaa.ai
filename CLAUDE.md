...
6. **All outbound Groq calls acquire from the token buckets in `core/pipeline_queue.py`.** Don't add a call site that bypasses rate limiting — the buckets are deliberately tuned below the free-tier ceiling on purpose (TRD §4.1).
